...
-- Global aggregation across ALL reports from ALL users
...
-- Groups by x_user_id, sums weighted_score across all qualified replies
...
BEGIN
    ...
    WITH ranked AS (
        SELECT
            r.x_user_id,
            -- Get the latest username and avatar (most recent reply)
            ...
            -- Use MAX for is_premium (if ever premium, show as premium)
            MAX(r.is_premium::int)::boolean AS is_premium,
            -- Use MAX for follower_count (latest/highest)
    ...
    SELECT
        ...
        ranked.total_score,
        ranked.reply_count,
        ROW_NUMBER() OVER (ORDER BY ranked.total_score DESC)::bigint AS rank
    FROM ranked
...
-- Groups by x_user_id, averages weighted_score across all qualified replies
...
BEGIN
    ...
    WITH ranked AS (
        ...
        WHERE r.to_be_included = true
          ...
          AND r.x_user_id IS NOT NULL
        GROUP BY r.x_user_id
        -- Require at least 1 qualified reply
        HAVING COUNT(*) >= 1
    )
    SELECT
        ranked.x_user_id,
        ...
        ranked.avg_score,
        ranked.reply_count,
        ROW_NUMBER() OVER (ORDER BY ranked.avg_score DESC, ranked.reply_count DESC)::bigint AS rank
    FROM ranked
    WHERE (p_search IS NULL OR ranked.username ILIKE '%' || p_search || '%')
    ORDER BY ranked.avg_score DESC, ranked.reply_count DESC
    LIMIT p_limit
    OFFSET p_offset;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Groups by original_author_username, averages reply scores for posts by that OP
...
BEGIN
    ...
    SELECT
        ...
        s.total_replies,
        c.report_count,
        ROW_NUMBER() OVER (ORDER BY s.avg_reply_score DESC, s.total_replies DESC)::bigint AS rank
    FROM op_stats s
    JOIN report_counts c ON c.op_username_key = s.op_username_key
    WHERE (p_search IS NULL OR s.op_username ILIKE '%' || p_search || '%')
    ORDER BY s.avg_reply_score DESC, s.total_replies DESC
    LIMIT p_limit
    OFFSET p_offset;
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Also grant to anon for public access (if desired)
